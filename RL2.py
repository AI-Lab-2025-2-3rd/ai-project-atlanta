...
def _save_best_path_plot(out_png, points, path_indices):
    ...
    # 노드 찍기
    plt.scatter(xs[1:], ys[1:], s=18, alpha=0.8, label="towns")
    plt.scatter([xs[0]], [ys[0]], s=60, marker="*", label="DEPOT")
    # 경로 라인
    px = [xs[i] for i in path_indices]
    py = [ys[i] for i in path_indices]
    plt.plot(px, py, linewidth=1.2, alpha=0.9, label="route")
...
class DQNAgent:
    ...
    def decay_epsilon(self):
        ...
        self.eps = EPS_START + (EPS_END - EPS_START) * frac
